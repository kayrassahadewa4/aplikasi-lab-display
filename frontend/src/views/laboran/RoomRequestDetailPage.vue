...
const requestItem = ref<RoomRequest | null>(null)
...
onMounted(() => {
  navStore.setBreadcrumbs([
    ...
    { label: 'Permohonan Pinjam', path: '/laboran/room-requests' },
...
const loadRequest = async () => {
  ...
  errorMessage.value = ''
  try {
    requestItem.value = await roomRequestService.getRoomRequestById(requestId.value)
  } catch (err: any) {
    errorMessage.value = err.message || 'Gagal memuat data permohonan pinjam lab'
  } finally {
...
const handleBack = () => {
  router.push('/laboran/room-requests')
}
...
<template>
  ...
  <!-- Error State -->
  <div v-else-if="errorMessage || !requestItem" class="flex items-center justify-center min-h-[400px]">
        ...
        <div>
          <div class="flex items-center gap-2.5 mb-1">
            <h1 class="text-2xl sm:text-[28px] font-extrabold text-text-primary tracking-tight leading-tight">
              {{ requestItem.activityName }}
            </h1>
            <span
              :class="[
                'inline-flex items-center gap-1 px-3 py-0.5 rounded-full text-xs font-extrabold border',
            ...
            >
              <Clock v-if="requestItem.status === 'PENDING'" :size="13" />
              <CheckCircle2 v-else-if="requestItem.status === 'APPROVED'" :size="13" />
              <XCircle v-else :size="13" />
              <span>{{ requestItem.status === 'PENDING' ? 'Menunggu' : requestItem.status === 'APPROVED' ? 'Disetujui' : 'Ditolak' }}</span>
            </span>
          </div>
          <p class="text-xs sm:text-sm text-text-muted font-medium">
            Diajukan oleh {{ requestItem.applicantName }} • Ruang Target: {{ requestItem.laboratoryCode }} ({{ requestItem.formattedRequestDate }})
          </p>
        </div>
      </div>
    </div>
      ...
      <div class="lg:col-span-8 space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 shadow-2xs p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
          <div class="flex items-center gap-4">
            <BaseAvatar
              :src="requestItem.applicantAvatar"
              :name="requestItem.applicantName"
              size="xl"
              class="shrink-0 ring-2 ring-brand-200/60 shadow-2xs"
            />
            <div>
              <h2 class="text-lg font-extrabold text-text-primary">{{ requestItem.activityName }}</h2>
              <p class="text-xs text-text-muted mt-0.5 font-medium">Pemohon: {{ requestItem.applicantName }} ({{ requestItem.applicantRole }}) • {{ requestItem.applicantEmail }}</p>
            </div>
          </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 shadow-2xs p-6 space-y-5">
          <div class="border-b border-gray-100 pb-3">
            <h3 class="text-base font-extrabold text-text-primary tracking-tight">Spesifikasi Permohonan Peminjaman</h3>
            <p class="text-xs text-text-muted">Ruangan target, rentang waktu reservasi, dan jumlah peserta.</p>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3.5">
            <div class="p-4 rounded-xl border border-gray-100 bg-surface/60 space-y-1">
              ...
              <p class="text-xs font-black text-text-primary truncate">{{ requestItem.formattedRequestDate }}</p>
            </div>
            ...
            <div class="p-4 rounded-xl border border-gray-100 bg-surface/60 space-y-1">
              ...
              <p class="text-xs font-mono font-black text-dark-green truncate">{{ requestItem.startTime }} – {{ requestItem.endTime }} WIB</p>
            </div>
            ...
            <div class="p-4 rounded-xl border border-gray-100 bg-surface/60 space-y-1">
              ...
              <p class="text-xs font-mono font-black text-dark-green truncate">{{ requestItem.laboratoryCode }} ({{ requestItem.laboratoryName }})</p>
            </div>
            ...
            <div class="p-4 rounded-xl border border-gray-100 bg-surface/60 space-y-1">
              ...
              <p class="text-xs font-black text-text-primary truncate">{{ requestItem.participantCount }} Mahasiswa {{ requestItem.className ? `(${requestItem.className})` : '' }}</p>
            </div>

            <div class="p-4 rounded-xl border border-gray-100 bg-surface/60 space-y-1">
              <div class="flex items-center gap-1.5 text-text-muted text-[11px] font-bold">
                <UserCheck :size="13" class="text-dark-green" />
                <span>Peran Pemohon</span>
              </div>
              <p class="text-xs font-bold text-dark-green truncate">{{ requestItem.applicantRole }}</p>
            </div>
            ...
            <div class="p-4 rounded-xl border border-gray-100 bg-surface/60 space-y-1">
              ...
              <p class="text-xs font-bold text-dark-green truncate">{{ requestItem.status === 'PENDING' ? 'Menunggu' : requestItem.status === 'APPROVED' ? 'Disetujui' : 'Ditolak' }}</p>
            </div>
          </div>
        </div>

        <!-- Purpose of Use Description Section -->
        <div v-if="requestItem.description" class="bg-white rounded-2xl border border-gray-200/80 shadow-2xs p-6 space-y-4">
          <div class="border-b border-gray-100 pb-3">
        ...
        <div v-if="requestItem.status === 'REJECTED' && requestItem.rejectionReason" class="bg-rose-50 rounded-2xl border border-rose-200 p-6 space-y-2">
          <h4 class="text-xs font-bold text-rose-800 flex items-center gap-2">
            <AlertCircle :size="16" class="text-rose-600" />
            <span>Alasan Penolakan</span>
          </h4>
          <p class="text-xs text-rose-700 leading-relaxed font-medium">{{ requestItem.rejectionReason }}</p>
        </div>
      </div>
      ...
      <div class="lg:col-span-4 space-y-5 lg:sticky lg:top-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 shadow-2xs p-5 space-y-3">
          <div class="flex items-center justify-between border-b border-gray-100 pb-2.5">
            ...
            <span class="px-2 py-0.5 rounded-full bg-brand-100 font-mono text-[10px] font-bold text-dark-green">
              {{ requestItem.laboratoryCode }}
            </span>
          </div>
          <div class="space-y-2 text-xs">
            <div class="flex justify-between py-1 border-b border-gray-50">
              <span class="text-text-muted">Ruangan Target</span>
              <span class="font-bold text-text-primary truncate max-w-[170px] text-right">{{ requestItem.laboratoryName }}</span>
            </div>
            <div class="flex justify-between py-1 border-b border-gray-50">
              <span class="text-text-muted">Tanggal Peminjaman</span>
              <span class="font-bold text-dark-green">{{ requestItem.formattedRequestDate }}</span>
            </div>
            <div class="flex justify-between py-1">
              <span class="text-text-muted">Sesi Waktu</span>
              <span class="font-mono font-bold text-text-primary">{{ requestItem.startTime }} – {{ requestItem.endTime }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-surface/60 rounded-2xl border border-gray-200/70 p-4 space-y-2 text-[11px] text-text-muted">
          <div class="flex justify-between">
            ...
            <span class="font-mono text-[10px] text-text-secondary truncate max-w-[140px]">{{ requestItem.id }}</span>
          </div>
          <div class="flex justify-between">
            <span>Diajukan Pada</span>
            <span class="font-medium text-text-secondary truncate max-w-[140px]">{{ requestItem.createdAt }}</span>
          </div>
        </div>
      </div>
    </div>
