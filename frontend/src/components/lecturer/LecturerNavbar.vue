...
const cachedName = ref(authStore.userName || 'Lecturer')
const cachedEmail = ref(authStore.userEmail || 'lecturer@univ.ac.id')
...
const lecturerName = computed(() => {
  ...
  return cachedName.value || 'Lecturer'
})

const lecturerEmail = computed(() => {
  if (authStore.userEmail) {
    cachedEmail.value = authStore.userEmail
    return authStore.userEmail
  }
  return cachedEmail.value || 'lecturer@univ.ac.id'
})

const handleLogout = async () => {
  // Navigate to login page first to prevent reactive layout flickers
  router.push('/login')
...
<template>
  ...
  >
    ...
    <!-- Right: Quick Links & Lecturer User Pill -->
    <div class="flex items-center gap-2 sm:gap-3">
      ...
      <!-- Room Requests & Live Notifications Dropdown -->
      <NotificationDropdown />

      <div class="h-5 w-px bg-gray-200 mx-1 hidden sm:block"></div>

      <!-- Lecturer User Profile Pill -->
      <router-link
        to="/lecturer/settings/profile"
      ...
      >
        <BaseAvatar :src="authStore.userAvatar" :name="lecturerName" size="sm" class="ring-2 ring-dark-green/30 group-hover:ring-dark-green" />
        <div class="hidden md:block">
          <div class="flex items-center gap-1.5">
            <p class="text-xs font-extrabold text-text-primary group-hover:text-dark-green transition-colors leading-none">{{ lecturerName }}</p>
            <span
          ...
          <p class="text-[10px] text-text-muted font-medium mt-0.5 truncate max-w-[130px]">
            {{ lecturerEmail }}
          </p>
        </div>
      </router-link>
