...
const isActive = (path?: string) => {
  ...
  if (path === '/lecturer') return route.path === '/lecturer'
...
<template>
  ...
  >
    ...
    <div class="h-16 flex items-center justify-between px-4 border-b border-emerald-800/40 shrink-0">
      <router-link to="/lecturer" class="flex items-center gap-2.5 overflow-hidden">
    ...
    <div v-if="!navStore.sidebarCollapsed" class="p-4 border-t border-emerald-800/40 shrink-0 bg-black/15">
      <div class="flex items-center gap-2">
        ...
        <p class="text-[11px] text-emerald-200/80 font-bold tracking-wide">Lecturer Mode</p>
      </div>
    </div>
  </aside>
</template>
